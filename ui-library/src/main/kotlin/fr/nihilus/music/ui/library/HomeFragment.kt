...
import fr.nihilus.music.ui.library.albums.AlbumsFragment
import fr.nihilus.music.ui.library.artists.ArtistsFragment
import fr.nihilus.music.ui.library.databinding.FragmentHomeBinding
import fr.nihilus.music.ui.library.playlists.PlaylistsFragment
import fr.nihilus.music.ui.library.tracks.AllTracksFragment
...
internal class HomeFragment : BaseFragment(R.layout.fragment_home) {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean = when (item.itemId) {
        ...
        R.id.action_cleanup -> {
            findNavController().navigate(
                NavDeepLinkRequest.Builder.fromUri("odeon://fr.nihilus.music.ui.cleanup/activity_cleanup".toUri())
                    .build()
            )
            true
        }
        ...
        R.id.action_settings -> {
            findNavController().navigate(
                NavDeepLinkRequest.Builder.fromUri("odeon://fr.nihilus.music.ui.settings/activity_settings".toUri())
                    .build()
            )
            true
        }
    ...
    private class MusicLibraryTabAdapter(fragment: Fragment) : FragmentStateAdapter(fragment) {
        ...
        fun getIcon(position: Int): Drawable? = when (position) {
            0 -> ContextCompat.getDrawable(context, CoreUiR.drawable.ic_audiotrack_24dp)
