...
internal class RestrictedMediaDaoTest {
    ...
    @Test
    fun `It should return unmodified tracks when having no exclusions`() = test.runWithin {
        val sut = RestrictedMediaDao(it, StubMediaDao, FakeExclusionDao())
        val restrictedTracks = sut.tracks.first()
        restrictedTracks.shouldContainExactly(ALGORITHM, DIRTY_WATER, MATTER_OF_TIME, RUN)
    }

    @Test
    fun `It should return unmodified albums when having no exclusions`() = test.runWithin {
        val sut = RestrictedMediaDao(it, StubMediaDao, FakeExclusionDao())
    ...
    @Test
    fun `It should return unmodified artists when having no exclusions`() = test.runWithin {
        val sut = RestrictedMediaDao(it, StubMediaDao, FakeExclusionDao())
    ...
    @Test
    fun `It should omit tracks that are in exclude list`() = test.runWithin {
        val sut = RestrictedMediaDao(
            it,
            StubMediaDao,
    ...
    @Test
    fun `Albums should subtract excluded tracks from track count`() = test.runWithin {
        val sut = RestrictedMediaDao(it, StubMediaDao, FakeExclusionDao(RUN.id))
    ...
    @Test
    fun `Albums having only excluded tracks should be omitted`() = test.runWithin {
        val sut = RestrictedMediaDao(it, StubMediaDao, FakeExclusionDao(MATTER_OF_TIME.id))
        val filteredAlbums = sut.albums.first()
        filteredAlbums.shouldContainExactly(CONCRETE_GOLD, SIMULATION_THEORY)
    }

    @Test
    fun `Artists should subtract excluded tracks and albums from count`() = test.runWithin {
        val sut = RestrictedMediaDao(it, StubMediaDao, FakeExclusionDao(MATTER_OF_TIME.id))
    ...
    @Test
    fun `Artists having only excluded tracks should be omitted`() = test.runWithin {
        val sut = RestrictedMediaDao(it, StubMediaDao, FakeExclusionDao(ALGORITHM.id))
        val filteredArtists = sut.artists.first()
        filteredArtists.shouldContainExactly(FOO_FIGHTERS)
    }

    @Test
    fun `It should update track list when exclude list changed`() = test.runWithin {
        val exclusions = FakeExclusionDao(MATTER_OF_TIME.id)
        val sut = RestrictedMediaDao(it, StubMediaDao, exclusions)
